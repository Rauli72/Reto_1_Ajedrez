public class Movimiento {
    ...
    public static boolean Peon(int fi, int ci, int ff, int cf,
                               String color, Tablero tablero) {

        int dir = color.equals("B") ? -1 : 1;
        int filaInicial = color.equals("B") ? 6 : 1;

        // Movimiento normal (1 adelante)
        if (ff == fi + dir && ci == cf &&
                tablero.getCasilla(ff, cf).getPieza() == null) {
            return true;
        }
        ...
        if (fi == filaInicial &&
                ff == fi + 2 * dir &&
                ci == cf &&
                tablero.getCasilla(fi + dir, ci).getPieza() == null &&
                tablero.getCasilla(ff, cf).getPieza() == null) {
            return true;
        }
        ...
        if (ff == fi + dir &&
                Math.abs(cf - ci) == 1 &&
                tablero.getCasilla(ff, cf).getPieza() != null &&
                !tablero.getCasilla(ff, cf).getPieza().getColor().equals(color)) {
            return true;
        }
